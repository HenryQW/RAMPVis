...
Being one of the four VIS volunteer teams, we received guidance from the RAMPVis team via regular virtual meetings. The RAMPVis team communicated with the SCRC modeling team regularly and provided us with the important information and data.
...
The plot supports brushing and linking, which enables the modelers to select a subset of the input parameters to focus on interesting configurations.
...
\begin{enumerate}
    \item The modelers found the parallel coordinates plot is useful in identifying outliers, and requested the incorporation of another one for the model outcomes.
    ...
    This resembles their usual workflow of analyzing multiple simulation outcomes.
    ...
    The motivation behind is to reduce the dimensionality and identify key parameters.
    We implemented this as shown in \Cref{fig:pca}.
    \item The modelers requested all views to be coordinated with each other, enabling the observation of relationships between input parameters and model outcomes through interaction:
    \begin{enumerate}
